...
void ParseDirectIPCustomTail(const uint8_t *pktdata, Direct_IP_Custom_Tail *tail)
{
    memcpy(tail, pktdata, sizeof(Direct_IP_Custom_Tail));
}
...
inline int PacketSetData(Packet *p, const uint8_t *pktdata, uint32_t pktlen)
{
    ...
    if (!initialized)
    {
        ConfGetBool("vars.is-line-no", &is_line_no);
        printf("is-line-no initialized with:%d\n", is_line_no);
        initialized = true;
    }
    ...
    if (is_line_no)
    {
        ...
        pktdata += 51;                      // Move the pktdata pointer past the 51 bytes we just copied
        pktlen -= 51;                       // Adjust pktlen to reflect the remaining length after copying

        // Copy the last 98 bytes to the pktdata field
        ParseDirectIPCustomTail(pktdata + pktlen - 98, &p->custom_tail);

        // Subtract the last 98 bytes, which will be ignored
        pktlen -= 98;
