...
static int AlertJson(ThreadVars *tv, JsonAlertLogThread *aft, const Packet *p)
{
    ...
    for (int i = 0; i < p->alerts.cnt; i++)
    {
        ...
        {
            int stream = (p->proto == IPPROTO_TCP) ? (pa->flags & (PACKET_ALERT_FLAG_STATE_MATCH | PACKET_ALERT_FLAG_STREAM_MATCH) ? 1 : 0) : 0;
...
char *bytesToHexString1(const uint8_t *data, size_t data_len)
{
    if (data == NULL || data_len == 0)
        return NULL;

    char *hex_str = (char *)malloc(2 * data_len + 1);
    if (hex_str == NULL)
    {
    ...
    {
        sprintf(&hex_str[2 * i], "%02X", data[i]);
    }
...
GoString ToGoString1(const char *str)
{
    GoString go_str;
    go_str.p = str;
    go_str.n = (ptrdiff_t)strlen(str);
    return go_str;
}

void OutPutAlertPacket(const Packet *p, const PacketAlert *pa)
{
    if (p == NULL)
        return;
    ...
    if (!initialized)
    {
        ConfGetInt("vars.forwarding-mode", &forwarding_mode);
        initialized = true;
    }

    uint8_t *pkt_data = GET_PKT_DATA(p);
    if (pkt_data == NULL)
        return;

    uint32_t pkt_len = p->pktlen;
    if (pkt_len == 0)
        return;

    GoInt direction = PKT_IS_TOSERVER(p) ? 1 : (PKT_IS_TOCLIENT(p) ? 2 : 0);

    char *signature = pa->s->msg ? pa->s->msg : "";
    char *category = pa->s->class_msg ? pa->s->class_msg : "";

    switch (forwarding_mode)
    {
    case 0:

        char *hex_string = bytesToHexString1(pkt_data, pkt_len);
        if (hex_string != NULL)
        {
            const char *l = NULL;
            const char *t = NULL;
            GoString line_no = {0};
            if (memcmp(p->line_no, zeros, 51) != 0)
            {
                l = bytesToHexString1(p->line_no, 51);
                line_no = ToGoString1(l);
            }
            else
            {
                line_no.p = NULL;
            ...
            GoString task_id = {0};
            if (memcmp(p->custom_tail.TaskID, zeros, TASK_ID_LENGTH) != 0)
            {
                t = bytesToHexString1(p->custom_tail.TaskID, TASK_ID_LENGTH);
                task_id = ToGoString1(t);
            }
            else
            {
                task_id.p = NULL;
            ...
            Entrance(ToGoString1(hex_string), line_no, task_id, direction, isalert, sid, class_type, emsg);
            free(hex_string);
            if (l != NULL)
            {
                free(l);
    ...
    case 1:
        PacketWrapper pw = {0};
        socket_client_send_packet(get_global_sock(), &pw);
        break;
    }
}

int AlertOutPut(ThreadVars *tv, JsonAlertLogThread *aft, const Packet *p)
{
    if (p->alerts.cnt == 0 && !(p->flags & PKT_HAS_TAG))
        return TM_ECODE_OK;
