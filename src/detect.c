                                   ...
                                   DetectRunScratchpad *scratch)
{
    ...
    while (engine != NULL)
    { // TODO could be do {} while as s->app_inspect cannot be null
...
char *bytesToHexString(const uint8_t *data, size_t data_len)
{
...
void OutPutDetectPacket(Packet *p)
{
    if (p == NULL)
        return;
    ...
    {
    case 0:
        const char *hex_string = bytesToHexString(pkt_data, pkt_len);
        if (hex_string != NULL)
        {
            GoString line_no = {0};
            GoString task_id = {0};
            ...
            {
                l = bytesToHexString(p->line_no, 51);
                line_no = ToGoString(l);
            }
            else
            {
                line_no.p = NULL;
            ...
            {
                t = bytesToHexString(p->custom_tail.TaskID, TASK_ID_LENGTH);
                task_id = ToGoString(t);
            }
            else
            {
                task_id.p = NULL;
            ...
            GoInt isalert = 0;
            GoInt sid = 0;
            GoString class_type = {0};
            GoString emsg = {0};

            Entrance(ToGoString(hex_string), line_no, task_id, direction, isalert, sid, class_type, emsg);
            free(hex_string);
            if (l != NULL)
            {
                free(l);
    ...
    case 1:
        PacketWrapper pw = {0};
        memcpy(pw.data, pkt_data, pkt_len);
        pw.delimiter = '\x1e';
        memcpy(pw.line_no, p->line_no, sizeof(pw.line_no));
        pw.direction = direction;
        printf("detect global_sock: %d line_no: %s pkt_len: %d\n", get_global_sock(), pw.line_no, pkt_len);
